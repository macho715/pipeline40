...
# Import column matcher for flexible column name matching
...
class DataSynchronizerV29:
    ...
    def _case_col(self, df: pd.DataFrame) -> Optional[str]:
        # Use flexible column matcher for Case No. column
    ...
    def _apply_master_no_sorting(
        ...
        no_col = find_column_flexible(master, ["no", "number", "num", "index", "번호"])
                        ...
                        def apply_formatting_inplace(
                            ...
                            try:
                                ...
                                for ch in getattr(self.ct, "changes", []) or []:
                                    ...
                                    if (
                                        target_cell.value is not None
                                        and str(target_cell.value).strip() != ""
                                    ):
