...
def calculate_derived_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Normalize column names and apply synonyms for robustness
    df.columns = normalize_columns(df.columns)
    df = apply_column_synonyms(df)
    working_df = df.copy()
    ...
    working_df = _ensure_handling_variants(working_df)
    return working_df
...
# Ensure both single-space and double-space variants exist for compatibility
def _ensure_handling_variants(df: pd.DataFrame) -> pd.DataFrame:
    if 'site handling' in df.columns and 'site  handling' not in df.columns:
        df['site  handling'] = df['site handling']
    if 'site  handling' in df.columns and 'site handling' not in df.columns:
        df['site handling'] = df['site  handling']
    return df
