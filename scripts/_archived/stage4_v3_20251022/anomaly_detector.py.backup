...
class RuleDetector:
    ...
    def status_mismatch(self, row: pd.Series, last_loc: Optional[str]) -> Optional[AnomalyRecord]:
        ...
        # Final_Location이 있으면 우선 사용 (Stage 3 계산 결과)
        final_loc = row.get("FINAL_LOCATION", None)
        if pd.isna(final_loc):
            final_loc = last_loc  # 없으면 계산된 값 사용
        
        if pd.isna(status) or pd.isna(final_loc):
            return None
        if str(status).strip() != str(final_loc).strip():
            return AnomalyRecord(
                case_id=str(row.get("CASE_NO","NA")),
                anomaly_type=AnomalyType.STATUS_MISMATCH,
                severity=AnomalySeverity.MEDIUM,
                description=f"STATUS_LOCATION({status}) ≠ 최종({final_loc})",
            )
        return None
