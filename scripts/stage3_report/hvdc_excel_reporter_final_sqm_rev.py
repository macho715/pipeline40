...
class CorrectedWarehouseIOCalculator:
    ...
    def _load_all_sheets_and_merge(self, file_path: Path, vendor_name: str) -> pd.DataFrame:
        """Load and merge all sheets from an Excel file.
        ...
        Returns:
            Merged DataFrame from all sheets
        """
        ...
        logger.info(f" Loading {len(xl.sheet_names)} sheets from {vendor_name} file")
        ...
        for sheet_name in xl.sheet_names:
            ...
            try:
                ...
                # Track source sheet for debugging
        ...
        if not all_dfs:
            logger.error(f"No valid sheets found in {file_path}")
            return pd.DataFrame()

        # Merge all sheets
        merged = pd.concat(all_dfs, ignore_index=True, sort=False)
        logger.info(f" Merged: {len(merged)} rows from {len(all_dfs)} sheets")
    ...
    def load_real_hvdc_data(self):
        ...
        try:
            # HITACHI 데이터 로드 (모든 시트)
            if self.hitachi_file.exists():
                logger.info(f" HITACHI 데이터 로드: {self.hitachi_file}")
                hitachi_data = self._load_all_sheets_and_merge(self.hitachi_file, "HITACHI")

                if hitachi_data.empty:
                    logger.warning(" HITACHI 데이터가 비어있습니다")
                else:
                    # [패치] 컬럼명 정규화 및 동의어 매핑
            ...
            # SIMENSE 데이터 로드 (모든 시트)
            if self.simense_file.exists():
                logger.info(f" SIMENSE 데이터 로드: {self.simense_file}")
                simense_data = self._load_all_sheets_and_merge(self.simense_file, "SIMENSE")

                if simense_data.empty:
                    logger.warning(" SIMENSE 데이터가 비어있습니다")
                else:
                    # [패치] 컬럼명 정규화 및 동의어 매핑
                    ...
                    try:
                        ...
                        for site in self.site_columns:
                            if site in row.index and pd.notna(row[site]):
                                ...
                                #  수정: 다음 날 이동만 출고로 인정
                                if site_date.date() == (
                                    warehouse_date.date() + timedelta(days=1)
                                ):  # 동일 날짜 제외
...
class HVDCExcelReporterFinal:
    ...
    def create_warehouse_monthly_sheet(self, stats: Dict) -> pd.DataFrame:
        ...
        # 컬럼 생성 (23열: 입고월 1 + 입고 10 + 출고 10 + 누계 2)
        columns = ["입고월"]

        # 입고 10개 창고 (Stage 1 정렬 순서)
        for warehouse in warehouse_display_names:
            columns.append(f"입고_{warehouse}")

        # 출고 10개 창고 (Stage 1 정렬 순서)
        for warehouse in warehouse_display_names:
    ...
    def generate_final_excel_report(self):
        ...
        # 입고일자 계산: 모든 창고 컬럼 중 가장 빠른 날짜
        warehouse_cols = self.calculator.warehouse_columns

        def get_earliest_warehouse_date(row):
            dates = []
            for wh in warehouse_cols:
                if wh in row.index and pd.notna(row[wh]):
                    try:
                        d = pd.to_datetime(row[wh])
                        if pd.notna(d):
                            dates.append(d)
                    except:
                        continue
        ...
        # 모든 원본 데이터에 입고일자 컬럼 추가
        hitachi_original["입고일자"] = hitachi_original.apply(get_earliest_warehouse_date, axis=1)
        siemens_original["입고일자"] = siemens_original.apply(get_earliest_warehouse_date, axis=1)
        combined_original["입고일자"] = combined_original.apply(get_earliest_warehouse_date, axis=1)

        print(f"    입고일자 컬럼 추가:")
        print(f"      - HITACHI: {hitachi_original['입고일자'].notna().sum()}건")
        print(f"      - SIEMENS: {siemens_original['입고일자'].notna().sum()}건")
        print(f"      - 통합: {combined_original['입고일자'].notna().sum()}건")
        ...
        # Excel 파일 생성 (수정 버전)
        excel_filename = f"HVDC_입고로직_종합리포트_{self.timestamp}_v3.0-corrected.xlsx"
